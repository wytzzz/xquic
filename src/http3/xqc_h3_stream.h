...
/*
XQC_HTTP3_STREAM_FLAG_NONE: 默认无标志。
XQC_HTTP3_STREAM_FLAG_TYPE_IDENTIFIED: 流类型已识别。
XQC_HTTP3_STREAM_FLAG_FC_BLOCKED: 流被流控阻塞。
XQC_HTTP3_STREAM_FLAG_READ_DATA_BLOCKED: 应用暂无法提供数据。
XQC_HTTP3_STREAM_FLAG_WRITE_END_STREAM: 应用结束发送数据。
XQC_HTTP3_STREAM_FLAG_QPACK_DECODE_BLOCKED: 流被QPACK解码阻塞。
XQC_HTTP3_STREAM_FLAG_READ_EOF: 远端发送FIN。
XQC_HTTP3_STREAM_FLAG_CLOSED: QUIC流关闭。
XQC_HTTP3_STREAM_FLAG_PUSH_PROMISE_BLOCKED: 流被PUSH_PROMISE阻塞。
XQC_HTTP3_STREAM_FLAG_PRIORITY_SET: 流优先级已设置。
XQC_HTTP3_STREAM_FLAG_RESET: 流被重置。
XQC_HTTP3_STREAM_NEED_WRITE_NOTIFY: 需要写通知。
XQC_HTTP3_STREAM_IN_READING: 正在读处理流程中。
XQC_HTTP3_STREAM_FLAG_ACTIVELY_CLOSED: 应用主动关闭流。
XQC_HTTP3_STREAM_FLAG_FIN_SENT: 已发送FIN。
